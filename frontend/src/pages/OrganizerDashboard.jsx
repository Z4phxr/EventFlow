...
import { parseToDate, formatDateForDisplay as formatDateUtil } from '../utils/date';
...
function OrganizerDashboard() {
  ...
  const formatDateForDisplay = (dateString) => formatDateUtil(dateString);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const d = typeof date === 'string' ? parseToDate(date) : date;
      ...
      const data = {
        ...formData,
        startAt: toLocalIsoWithOffset(formData.startAt),
        endAt: toLocalIsoWithOffset(formData.endAt),
  ...
  const handleCancelEvent = async (eventId) => {
    if (!window.confirm('Are you sure you want to cancel this event? This will mark it as CANCELLED.')) return;
    setActionLoading(true);
    try {
      ...
      setMessage({ text: err.response?.data?.message || 'Failed to cancel event', type: 'error' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const formatRegistrationDate = (dateString) => formatDateUtil(dateString);
